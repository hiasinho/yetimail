...
Press `x` to archive or `Shift+X` to trash the highlighted row in list mode or the displayed message in reader mode, immediately without confirmation. These actions use the same safe folder-move routing as the picker, never `message delete`; `Shift+X` is a no-op in Trash, and `x` is a no-op in Archive. Missing or ambiguous roles fail closed. If discovery is not ready, the action loads folders and asks you to retry; it never queues a move for later. Actions are blocked while busy, closed, or in help, folder picker, or deletion confirmation. Real moves require an explicit account.
